...
class ControleSupabaseUser extends BaseAuthUser {
  ControleSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> controleSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ControleSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
